...
type
  ...
  protected
    ...
    function GetLocked: boolean; {$ifdef HASINLINE}inline; {$endif}
  ...
  public
    constructor Create(const Host: RawUtf8; Port: integer = MONGODB_DEFAULTPORT;
      ...
      const SecondaryPortCsv: RawUtf8 = ''); reintroduce;
  ...
  public
    ...
    constructor Create(aClient: TMongoClient; const aDatabaseName: RawUtf8); reintroduce;
  ...
  public
    ...
    constructor Create(aDatabase: TMongoDatabase; const aCollectionName: RawUtf8); reintroduce;
...
begin
  if {$ifdef MONGO_OLDPROTOCOL}
     ...
     {$endif MONGO_OLDPROTOCOL}
     (fRequestID = 0) then
    EMongoException.CreateUtf8('Missing proper %.Create() call', [self]);
...
begin
  if high(CursorIDs) < 0 then
    EMongoException.CreateUTF8('Invalid %.Create([]) call', [self]);
...
begin
  ...
  if not BsonVariantType.IsOfKind(fCommand, betDoc) then
    EMongoException.CreateUtf8('%.Create: command?', [self]);
...
procedure TMongoMsg.ToJson(W: TTextWriter; Mode: TMongoJsonMode);
begin
  ...
  if AddMongoJson(fCommand, W, modMongoShell, 1024) then
    W.Add(['.', '.', '.']) // huge Command has been truncated after 1KB
  else
    W.Add('}')
end;
...
begin
  ...
  if (len < SizeOf(TOpMsgHeader)) or
    ...
    EMongoException.CreateUTF8('%len=%', [_E, len]);
  ...
  begin
    ...
    if cmp.OriginalOpcode <> OP_MSG then
      EMongoException.CreateUTF8('%orig=%', [_E, cmp.OriginalOpcode]);
    if cmp.CompressorId <> ZLIB_COMPRESSORID then
      EMongoException.CreateUTF8('%compressor=%', [_E, cmp.CompressorId]);
    if (cmp.UncompressedSize < 5) or
       (cmp.UncompressedSize > BSON_MAXDOCUMENTSIZE) then
      EMongoException.CreateUTF8('%size=%', [_E, cmp.UncompressedSize]);
    ...
    if UncompressMem(
      ...
      EMongoException.CreateUTF8('%zlib decompression', [_E]);
    msg := pointer(fReply);
  ...
  begin
    ...
    EMongoException.CreateUTF8('%OpCode=%', [_E, msg.Header.OpCode]);
  if msg.SectionKind <> mmkBody then
    EMongoException.CreateUTF8('%kind=%', [_E, ord(msg.SectionKind)]);
  ...
  begin
    ...
    QueryPerformanceMicroSeconds(fElapsedMicroSec);
    {$else}
    QueryPerformanceCounter(fElapsedMicroSec);
    {$endif LINUX}
    dec(fElapsedMicroSec, StartMS);
  end;
end;
...
begin
  if cardinal(index) >= cardinal(fDocumentCount) then
    EMongoException.CreateUTF8('TMongoReplyCursor.GetDocument(index %>=%)',
...
begin
  ...
  if result <> length(Dest) then
    EMongoException.Create('Invalid opReply Documents');
end;
...
var
  item: variant;
begin
  if cardinal(Dest.VarType) <> cardinal(DocVariantVType) then
...
begin
  if aClient = nil then
    EMongoException.CreateUTF8('%.Create(nil)', [self]);
  fClient := aClient;
...
begin
  ...
  try
    fSocket := TCrtSocket.Open(fServerAddress,UInt32ToUtf8(fServerPort),
      cslTCP,Client.ConnectionTimeOut,mcoTls in Client.Options);
  except
    on E: Exception do
      EMongoException.CreateUTF8('%.Open unable to connect to MongoDB server %: % [%]',
...
procedure TMongoConnection.SendAndGetReply(
  ...
  begin
    if not fSocket.TrySockRecv(Buf,Len) then
      try
        Close;
      finally
...
begin
  inherited Create(UTF8ToString(aMsg));
...
begin
  if (self = nil) or
     ...
     (UserName = '') or
     (PassWord = '') then
    EMongoException.CreateUtf8('Invalid %.OpenAuth("%") call',
      [self, DatabaseName]);
  result := fDatabases.GetObjectFrom(DatabaseName);
...
begin
  ...
  if (self = nil) or
     ...
     (UserName = '') or
     (Digest = '') then
    EMongoException.CreateUtf8('Invalid %.Auth("%") call',
      [self, DatabaseName]);
  ...
  begin
    ...
    if err <> '' then
      EMongoException.CreateUtf8('%.OpenAuthCR("%") step1: % - res=%',
    ...
    err := fConnections[ConnectionIndex].RunCommand(DatabaseName, bson, res);
    if err <> '' then
      EMongoException.CreateUtf8('%.OpenAuthCR("%") step2: % - res=%',
        [self, DatabaseName, err, res]);
  end
  else
  begin
    ...
    begin
      ...
      if err <> '' then
        EMongoException.CreateUtf8('%.OpenAuthSCRAM("%") step3: % - res=%',
          [self, DatabaseName, err, res]);
    end;
  end;
end;
...
begin
  fClient := aClient;
  fName := aDatabaseName;
  fCollections := TRawUtf8List.Create(
...
begin
  ...
  if fDatabase.Client.ServerBuildInfoNumber < 2020000 then
    EMongoException.Create('Aggregation needs MongoDB 2.2 or later');
...
begin
  ...
  if DocVariantData(Keys)^.kind <> dvObject then
    EMongoException.CreateUtf8('%[%].EnsureIndex(Keys?)',
      [self,
      FullCollectionName]);
    ...
    with _Safe(Keys)^ do
      ...
      begin
        ...
        if order = -1 then
          ...
          EMongoException.CreateUtf8('%[%].EnsureIndex() on order {%:%}',
...
begin
  ...
  if not VarIsEmptyOrNull(Projection) then
    if VarIsString(Projection) then
      EMongoException.CreateUtf8('%.DoFind: unsupported string', [self])
    else
  ...
  with _Safe(res)^ do
    ...
    if GetAsArray('writeErrors', errors) then
      if ContOnFail then
        ...
        EMongoException.CreateUTF8('%.% on % failed as %',
...
begin
  ...
  if length(Updates) <> length(Queries) then
    EMongoException.CreateUTF8('%.Update(%<>%)', [self, n, length(Updates)]);
  SetLength(upd, n);
  for i := 0 to n - 1 do
...
begin
  ...
  begin
    ...
    if deleteIndex >= 0 then // restore _id in place
      _Safe(Document)^.AddValue('_id', oid, false);
  end;
end;
