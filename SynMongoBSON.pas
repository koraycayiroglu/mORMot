...
type
  ...
  public
    ...
    procedure ToJSON(W: TTextWriter; const Value: PVarData);
    /// variant serialization will use modMongoStrict JSON-compatible mode
    ...
    function IsVoid(const V: TVarData): boolean;
...
type
  ...
  public
    ...
    function AddMongoJSON(W: TTextWriter; Mode: TMongoJsonMode = modMongoStrict;
...
type
  ...
  public
    ...
    /// write a string, detecting TTextWriter.AddDateTime/WrBase64 patterns
...
// specific MongoDB objects but you may use modMongoShell if needed
// - returns false on success, true if MaxSize has been reached
function AddMongoJson(const Value: variant; W: TTextWriter;
...
begin
  ...
  SetString(RawByteString(res.VBlob), PAnsiChar(@Bits), SizeOf(TDecimal128));
end;
...
begin
  b := @value;
  if cardinal(b^.VType) = (varVariant or varByRef) then
...
begin
  ...
  with GlobalBSONObjectID do begin
    TAESPRNG.Main.FillRandom(@InitialCounter,3);
    with ExeVersion do
...
procedure TBSONVariant.ToJSON(W: TTextWriter; const Value: PVarData);
var
  item: TBsonElement;
begin
...
function TBsonElement.AddMongoJson(W: TTextWriter; Mode: TMongoJsonMode;
...
begin
  ...
  case integer(Kind) of
    ...
    ord(betDoc),
    ord(betArray):
      result := BSONListToJSON(Data.DocList, Kind, W, Mode);
    ord(betObjectID):
      begin
...
begin
  v := @aValue;
  while cardinal(v.VType) = (varVariant or varByRef) do
    v := v.VPointer;
        ...
        case Kind of
          ...
          betInt32:
            if not VariantToInteger(aValue, PInteger(Element)^) then
              EBsonException.Create('TBsonElement.FromVariant(betInt32)');
          betInt64:
            if not VariantToInt64(aValue, PInt64(Element)^) then
              EBsonException.Create('TBsonElement.FromVariant(betInt64)')
...
str:    Kind := betString;
    ...
    begin
      ...
      begin
        ...
        if ElementBytes < 0 then
          EBsonException.CreateUTF8('TBsonElement.FromVariant(bson,%)',
            [ToText(Kind)^]);
      end;
    end
    ...
    begin
      if vdoc.Kind = dvObject then
        Kind := betDoc
      else if vdoc.Kind = dvArray then
        Kind := betArray
      else
        EBsonException.CreateUTF8('TBsonElement.FromVariant(doc,%)',
          [ToText(vdoc.Kind)^]);
      aTemp := Bson(vdoc^);
      FromBson(pointer(aTemp));
      if ElementBytes < 0 then
        EBsonException.CreateUTF8('TBsonElement.FromVariant(docbson,%)',
          [ToText(Kind)^]);
    end
    else
      EBsonException.CreateUTF8(
...
begin
  if valueLen >= 3 then
    if (PInteger(value)^ and $ffffff = JSON_BASE64_MAGIC) and
    ...
    begin
      ...
      exit;
            ...
            ((PInteger(value)^ and $ffffff = JSON_SQLDATE_MAGIC) and
             Iso8601CheckAndDecode(value + 3, valueLen - 3, dt)) then
    begin
      // recognized TTextWriter.AddDateTime() pattern into betDateTime
...
begin
  ...
  else if doc.Kind = dvArray then
    BsonWrite(name, betArray)
  else
    EBsonException.Create('Undefined nested document');
  BsonWriteDoc(doc);
end;
...
begin
  if not (kind in [betDoc, betArray]) then
    EBsonException.Create('BsonDocumentBegin(?)');
  BsonWrite(name, kind);
  BsonDocumentBegin;
end;
...
begin
  ...
  begin
    ...
    o := fDocumentStack;
    if o = 0 then
      EBsonException.CreateUTF8('Unexpected %.BsonDocumentEnd', [self]);
    dec(o);
    fDocumentStack := o;
...
begin
  ...
  case vt of
    ...
    varString:
      // will recognize TTextWriter.AddDateTime/WrBase64 patterns
    ...
    begin
      ...
      try
        ...
        {$endif HASVARUSTRING}
          ...
          begin
            VariantSaveJSON(value, twJsonEscape, RawUtf8(tmp));
            json := tmp;
    ...
    if vt <> DocVariantVType then
      ...
      begin
        ...
        if TotalWritten > BSON_MAXDOCUMENTSIZE then
          EBsonException.CreateUTF8('%.BsonWriteDoc(size=%>max %)',
        ...
        if like[1] = '%' then
          if len = 1 then
            ...
            if len = 2 then
              ...
              like := copy(like,2,len-2)    // LIKE '%a%' -> /a/
          else
            // LIKE '%a'  -> /a$/
    ...
    case Json^ of
      ...
      begin
        ...
        if (PInteger(Value)^ and $ffffff=JSON_SQLDATE_MAGIC) and
          ...
          BsonWriteUtf8OrDecode(name,Value,ValueLen);
      end;
    end;
  if TotalWritten > BSON_MAXDOCUMENTSIZE then
    EBsonException.CreateUTF8('Unexpected %.BsonWriteDoc(size=% > max=%)',
      [self, TotalWritten, BSON_MAXDOCUMENTSIZE]);
end;
...
begin
  if (BSON = nil) or
    ...
    EBsonException.Create('Incorrect supplied BSON document content');
...
begin
  if Option = asBsonVariant then
    EBsonException.Create('BsonToDoc: Option not allowed');
  VarClear(Result);
...
function BsonListToJson(BsonList: PByte; Kind: TBsonElementType; W: TTextWriter;
...
begin
  ...
  case Kind of
    ...
    EBsonException.CreateUTF8('BsonListToJson(Kind=%)', [ord(Kind)]);
  end;
